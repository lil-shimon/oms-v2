...
func testMaintenancesExists(t *testing.T) {
	...
	e, err := MaintenanceExists(ctx, tx, o.ID)
	if err != nil {
...
func testMaintenancesFind(t *testing.T) {
	...
	maintenanceFound, err := FindMaintenance(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
var (
	maintenanceDBTypes = map[string]string{`ID`: `int`, `Date`: `datetime`, `Title`: `varchar`, `Address`: `varchar`, `Note`: `text`, `WorkingHours`: `double`, `DeletedAt`: `timestamp`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `Count`: `int`}
	_                  = bytes.MinRead
)
