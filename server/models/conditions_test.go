...
func testConditionsExists(t *testing.T) {
	...
	e, err := ConditionExists(ctx, tx, o.ID)
	if err != nil {
...
func testConditionsFind(t *testing.T) {
	...
	conditionFound, err := FindCondition(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
var (
	conditionDBTypes = map[string]string{`ID`: `int`, `Name`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_                = bytes.MinRead
)
...
func testConditionsUpsert(t *testing.T) {
	...
	if len(mySQLConditionUniqueColumns) == 0 {
